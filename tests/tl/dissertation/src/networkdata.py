...
class NetworkData:
    def __init__(self, net_fp):
        ...
        self.n = self.getN()
        self.incidentIntersections = self.getIncidentIntersections()
        self.incidentEdges = self.getIncidentEdges()
        self.incident = False
        self.incidentCountDown = 0
        self.nIncident = 0
        print("SUCCESSFULLY GENERATED NET DATA")

    def getN(self):
        intersections = [key for key in self.intersection_data.keys()]
        intersections.sort()
        
        last = intersections[-1]
        _, n, _ = re.split('(\d+)',last)
        return int(n) + 1

    def getIncidentIntersections(self):
        ret = []
        intersections = [key for key in self.intersection_data.keys()]
        intersections.sort()
        n = self.n
        intersections = intersections[n:n*n-n]
    ...
    def getIncidentEdges(self):
        incidentInts = self.getIncidentIntersections()
        nodes = self.node_data
        es = []

        for node in incidentInts:
            for e in nodes[node]["incoming"]:
                es.append(e)
            for e in nodes[node]["outgoing"]:
                es.append(e)
        return es

    def addInfotoIncidentEdges(self):
        edges = self.edge_data
        ret = {edge:{} for edge in nd.incidentEdges}
        for key in ret.keys():
            ret[key]["length"] = edges[key]["length"]
